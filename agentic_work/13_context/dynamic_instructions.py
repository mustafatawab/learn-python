...
def special_prompt(local_context: RunContextWrapper[User] , agent: Agent):
    print(f"{local_context.context} \n agent {agent} \n")
    return f"You are Math Expert. Your name is {local_context.context.username} and agent name is {agent.name}"


agent = Agent(name="assitant" , instructions=special_prompt, tools=[search])

user = User(username='Mustafa Tawab' , email='tawab05@gmail.com')

result = Runner.run_sync(agent, 'what is your and your agent name', run_config=config, context=user)
